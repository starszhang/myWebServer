...
public:
    ...
    //基类覆写的，IO复用统一的接口
    virtual TimeStamp poll(int timeoutMs, ChannelList *activeChannels) = 0; //在指定的超时时间内等待事件发生，并将发生事件的通道填充到activeChannels中。
...
protected:
    using ChannelMap = std::unordered_map<int, Channel*>; //存储所有被监视的Channel，key:socketfd, value:Channel*
